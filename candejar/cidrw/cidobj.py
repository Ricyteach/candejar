...
from typing import List, Any, Optional, NewType, Sequence, Type, TypeVar, MutableSequence, Generic, Generator, Union
...
SEQ_NAME_DICT = {"pipe_groups": A2, "nodes": C3, "elements": C4, "boundaries": C5, "materials": D1, "factors": E1}
...
class CidSubObj(Generic[CidSubLine, FEAObj]):
    ...
    @property
    def iter_line_objs(self) -> Generator[CidLine]:
    ...
    def fea_obj(self) -> FEAObj:
        ...
        return self.type_(**{k:v for line_obj in self.iter_line_objs for k,v in asdict(line_obj).items() if k in field_names})
...
class CidSubSeq(Sequence[CidSubObj], Generic[CidSubLine, FEAObj]):
    ...
    def __init__(self, cid_obj: "CidObj", line_type: Type[CidSubLine]) -> None:
        ...
        self.type_: Type[FEAObj] = TYPE_DICT[line_type]
    ...
    def __getitem__(self, idx: int) -> CidSubObj:
        self.seq[idx] = next(iter(self.seq[idx:idx]), CidSubObj(self, idx))
        return self.seq[idx]
...
class CidObj:
    ...
    pipe_groups: Optional[CidSubSeq] = field(default=None, init=False)  # pipe groups
    nodes: Optional[CidSubSeq] = field(default=None, init=False)
    elements: Optional[CidSubSeq] = field(default=None, init=False)
    boundaries: Optional[CidSubSeq] = field(default=None, init=False)
    soilmaterials: Optional[CidSubSeq] = field(default=None, init=False)  # soil element materials
    interfmaterials: Optional[CidSubSeq] = field(default=None, init=False)  # interface element materials
    factors: Optional[CidSubSeq] = field(default=None, init=False)  # lrfd step factors
